...
func main() {
	...
	publicKeyBytes, err := encodePublicKeyToOpenSSH(&privateKey.PublicKey, "generated by giantcloak")
	if err != nil {
		panic(err)
	}
	...
	jsonFile, err := os.OpenFile(jsonFilePath, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0o644)
	if err != nil {
...
func encodePublicKeyToOpenSSH(publicKey *rsa.PublicKey, comment string) ([]byte, error) {
	...
	publicKeyBytes := []byte(fmt.Sprintf("%s %s", string(authorizedKey), comment))
